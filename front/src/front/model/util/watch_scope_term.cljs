...
(defn build-query-table-and-keys [& [{:keys [name-table query-keys-additional] :as params-optional}]]
  (util.core/build-query-table-and-keys
   (or name-table @#'name-table) query-keys params-optional))

#_(defn build-query-table-and-keys [& [{:keys [name-table query-additional-keys]}]]
    (format "%s {%s}"
